...
class ParameterOptimizer:
    ...
    def optimize(self,
        ...
        For nucleotide sequences, sweeps (reward, penalty) scoring pairs in addition
        to word_size and gap penalties. For protein, only word_size and gap penalties
        are swept (blastp uses fixed substitution matrices).

        Sweep order: word_size → scoring (nucl only) → gap penalties
        """
        ...
        # Scoring pairs to sweep (nucleotide only)
        scoring_pairs = DEFAULT_SCORING_PAIRS if is_nucleotide else [(None, None)]

        # Get valid gap penalty combinations — start with the full set for initial steps
        ...
        with BlastRunner(self.seq_type) as runner:
            ...
            # Step 2: Optimize scoring (reward, penalty) — nucleotide only
            if is_nucleotide:
                if verbose:
                ...
                for reward, penalty in scoring_pairs:
                    ...
                    # Use the middle valid gap combo for this scoring pair to avoid
                    # passing gap penalties that BLAST rejects for this reward/penalty
                ...
                # Re-filter gap combos to those valid for the chosen scoring pair
                valid_for_scoring = BLASTN_SCORING_COMBOS.get(
                    (current_reward, current_penalty), initial_gap_combos
                )
                filtered_gap_combos = [
                    (go, ge) for go, ge in valid_for_scoring
                    if gap_open_min <= go <= gap_open_max and gap_extend_min <= ge <= gap_extend_max
                ] or valid_for_scoring
                # Update current gap values to a valid combo for the new scoring
        ...
        # Collect final optimal parameters
