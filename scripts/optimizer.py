...
# Valid blastn gap combos per (reward, penalty) scoring pair
...
# Scoring pairs swept during nucleotide optimization
...
class ParameterOptimizer:
    ...
    def optimize(self,
                 gap_open_range: Tuple[int, int, int],
        ...
        For nucleotide sequences, also sweeps blastn match/mismatch scoring pairs
        (reward, penalty) and restricts gap penalty combinations to those valid
        for the chosen scoring.

        coverage_threshold is passed through to the final run_all_pairwise call.
        Set to 0.0 (via --no-coverage-guard) to disable the coverage guard.
        """
        ...
        if not filtered_initial_gaps and verbose:
            print(f"  Warning: Requested gap ranges have no valid combinations. "
                  f"Using all valid combinations for {self.seq_type.name}.")
        ...
        with BlastRunner(self.seq_type) as runner:
            ...
            if is_nucleotide:
                ...
                for reward, penalty in scoring_pairs:
                    ...
                    all_results.append(({'word_size': current_word_size,
                                         'reward': reward, 'penalty': penalty,
                                         'gap_open': sweep_go, 'gap_extend': sweep_ge},
                                        score))
                    if verbose:
                        print(f" -> {score:.1f}")
            ...
            optimization_trace['gap_penalties'] = {
                'method': method, 'combinations': filtered_gap_combos,
