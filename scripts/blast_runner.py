...
class BlastRunner:
    ...
    def run_all_pairwise_multi_hsp(self, sequences: List[Sequence],
        ...
        Used by synteny-aware MSA: each HSP becomes a positional anchor locked
        to the reference frame. Pairs with no hit are absent from the result.
        Coverage guard is not applied here — low single-HSP coverage is expected
        and resolved by chaining multiple HSPs.

        Returns:
            Dict mapping (query_id, subject_id) -> List[BlastHit] sorted by query_start.
        """
        if not self._check_blast_installed():
            raise RuntimeError("BLAST+ not found.")
        ...
        if verbose:
            print(f"  Creating BLAST database (synteny mode, max_hsps={max_hsps})...")

        all_db = self.create_database(sequences, db_name='all_seqs_synteny')

        query_path = Path(self.temp_dir) / 'queries_synteny.fasta'
        with open(query_path, 'w') as f:
            for seq in sequences:
                f.write(f">{seq.id}\n{seq.seq}\n")
